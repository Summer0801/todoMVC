...
(function() {
  ...
  var _ = (function(_) {
    ...
    //默认情况下，下划线使用erb样式的模板分隔符，更改下列模板设置以使用替代分隔符。
    _.templateSettings = {
      evaluate: /<%([\s\S]+?)%>/g,
      interpolate: /<%=([\s\S]+?)%>/g,
      escape: /<%-([\s\S]+?)%>/g
    ...
    // 在定制“templateSettings”时，如果你不想定义一个插值、评估或转义正则表达式，我们需要一个保证不匹配。
    var noMatch = /(.)^/;

    // Certain characters need to be escaped so that they can be put into a
    // string literal.
    // 某些字符需要被转义，以便将它们放入字符串文字中。
    var escapes = {
      "'": "'",
      "\\": "\\",
      "\r": "r",
    ...
    // JavaScript微模板，类似于John Resig的实现。下划线模板处理任意分隔符，保留空白，并正确地转义插入代码中的引号。
